...
service_token = ctx.changeControl.args.get("auth")
filename = ctx.changeControl.args.get("filename")
expiry = ctx.changeControl.args.get("expiry") 
# expiry units are d, h, or m: for days hours or minutes respectively.

# only include nodes that aren't empty.
secondary_cluster = []
for node in [ 
    ctx.changeControl.args.get("backup_cvp_1"), 
    ctx.changeControl.args.get("backup_cvp_2"), 
    ctx.changeControl.args.get("backup_cvp_3") ]:
